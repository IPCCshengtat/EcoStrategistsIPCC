...
def runopt(elec_requirement, elec_nat, elec_POME, elec_solar, cost_nat, cost_POME, cost_solar, 
    ...
    class PSEProblem(ElementwiseProblem):
        ...
        def _evaluate(self, x, out, *args, **kwargs):
            ...
            f1 = (cost_nat*x1*(x1*w_cost_nat - x1) + cost_POME*x2*abs(x2*w_cost_POME - x2) + cost_solar*x3*abs(x3*w_cost_solar - x3))
            f2 = (co2_nat*x1*(x1*w_co2_nat - x1) + co2_pome*x2*abs(x2*w_co2_POME - x2) + co2_solar*x3*(x3*w_co2_solar - x3))
    ...
    algorithm = NSGA2(pop_size=30, # 300
    ...
    termination = get_termination("n_gen", 50) # 150
    print("Something is written here")
