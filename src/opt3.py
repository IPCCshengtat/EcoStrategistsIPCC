...
def runopt(elec_requirement, elec_nat, elec_POME, elec_solar, cost_nat, cost_POME, cost_solar, 
    ...
    class PSEProblem(ElementwiseProblem):
        ...
        def _evaluate(self, x, out, *args, **kwargs):
            ...
            f1 = (cost_nat*x1*(x1*w_cost_nat - x1 + 1) + cost_POME*x2*abs(x2*w_cost_POME - x2 + 1) + cost_solar*x3*abs(x3*w_cost_solar - x3 + 1))/cost_solar
            f2 = (co2_nat*x1*(x1*w_co2_nat - x1 + 1) + co2_pome*x2*abs(x2*w_co2_POME - x2 + 1) + co2_solar*x3*(x3*w_co2_solar - x3 + 1))/co2_nat
